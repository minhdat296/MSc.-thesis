\section{Constructing Yangian extended toroidal Lie algebras}
    ...
    \subsection{Non-uniqueness of Yangian extended toroidal Lie brackets}
        ...
        \todo[inline]{I decided to not use the notation $\der_{\gamma}(A)$ nor the term "$\gamma$-extended Lie algebras", which was confusing me while writing the classification section, since $\gamma$ does not vary for us.}
        ...
        \begin{itemize}
            \item firstly, it makes sense to regard $\d_{[2]}$ as a Lie subalgebra of $\der_k(A)$ (cf. proposition \ref{prop: lie_bracket_on_orthogonal_complement_of_toroidal_centre}), and 
        ...
        \begin{definition}[Formal residue] \label{def: formal_residues}
            The \textbf{formal residue} of any element $f(v_1, ..., v_n) \in k[v_1^{\pm 1}, ..., v_n^{\pm 1}]$ is then given by:
                $$\Res_{(v_1, ..., v_n) = (0, ..., 0)}(f) := ( \Res_{v_n = 0} \circ ... \circ \Res_{v_1 = 0} )( f(v_1, ..., v_n) )$$
            Each map:
                $$\Res_{v_i = 0}: k[v_i, ..., v_n] \to k[v_{i + 1}, ..., v_n]$$
            is takes as input an element $f(v_i, ..., v_n) \in k[v_{i + 1}, ..., v_n]$ and outputs the $k[v_{i + 1}, ..., v_n]$-coefficient of the term with $v_i^{-1}$, or $0$ if there is no such term. 
        \end{definition}
        \begin{remark}[Taking formal residues is linear]
            ...
            From this, one sees that:
                $$\Res_{(v_1, ..., v_n) = (0, ..., 0)}: k[v_1, ..., v_n] \to k$$
            is a linear map.
        \end{remark}
        \begin{proposition}[Intrinsic description of $\d_{[2]}$] \label{prop: intrinsic_description_of_yangian_div_zero_vector_fields}
            Let us regard $\d_{[2]}$ as a Lie algebra, with the commutator bracket $[-, -]$. $\d_{[2]}$ is then equal to the Lie algebra:
                $$\der_k(A)_{(v, t) = (0, 0)} := \{ D \in \der_k(A) \mid \forall f \in A: \Res_{(v, t) = (0, 0)}( v^{-1} D(f) ) = 0 \}$$
        \end{proposition}
            \begin{proof}
                Firstly, we must check that the subset $\der_k(A)_{(v, t) = (0, 0)}$ of $\der_k(A)$ is indeed a vector subspace, and secondly, we must check that it is closed under the commutator bracket $[-, -]$, qualifying it as a Lie subalgebra of the latter. To prove the first assertion, simply take any linear combination:
                    $$a D + b D'$$
                of elements $D, D' \in \der_k(A)_{(v, t) = (0, 0)}$, and then consider the following for any arbitrary $f \in A$:
                    $$\Res_{(v, t) = (0, 0)}( v^{-1} ( a D + b D' )(f) ) = a \Res_{(v, t) = (0, 0)}( v^{-1} D(f) ) + b\Res_{(v, t) = (0, 0)}( v^{-1} D'(f) ) = 0$$
                To prove the second assertion, pick arbitrary elements $D, D' \in \in \der_k(A)_{(v, t) = (0, 0)}$ again, and then consider the following for any $f \in A$:
                    $$\Res_{(v, t) = (0, 0)}( v^{-1} ([D, D'])(f) ) = \Res_{(v, t) = (0, 0)}( v^{-1} DD'(f) ) - \Res_{(v, t) = (0, 0)}( v^{-1} D'D(f) ) = 0$$

                Now, to prove that:
                    $$\d_{[2]} = \der_k(A)_{(v, t) = (0, 0)}$$
                it shall suffice to prove that the underlying vector spaces are equal. Firstly, to prove that:
                    $$\d_{[2]} \subset \der_k(A)_{(v, t) = (0, 0)}$$
                ...
                for all $f \in A$, which we might as well take to be a basis element $v^m t^p$ (for some $(m, p) \in \Z^2$). In lemma \ref{lemma: derivation_action_on_multiloop_algebras}, we have already seen how the elements $D(v^m t^p) \in A$ are given for $D \in \d_{[2]}$ a basis vector, from which it can be easily seen that indeed, $-\Res_{(v, t) = (0, 0)}( v^{-1} D(f) ) = 0$. Conversely, to prove that:
                    $$\d_{[2]} \supset \der_k(A)_{(v, t) = (0, 0)}$$
                we shall need to show that any $D \in \der_k(A)_{(v, t) = (0, 0)}$ can be written as a linear combination of the elements of the basis $\{D_{r, s}\}_{(r, s) \in \Z^2} \cup \{D_v, D_t\}$. To that end, consider some:
                    $$D := \sum_{(a, b) \in \Z^2} \lambda_{a, b} v^a t^b \del_v + \sum_{(a', b') \in \Z^2} \mu_{a', b'} v^{a'} t^{b'} \del_t \in \der_k(A)_{(v, t) = (0, 0)}$$
                ...
                From this, we infer that:
                    $$
                        ...
                        \end{aligned}
                    $$
                Since we assumed that $D \in \der_k(A)_{(v, t) = (0, 0)}$, we must have that:
                ...
                for all $(m, p) \in \Z^2$. By comparing this with how the elements $D_{r, s}, D_v, D_t$ are given in terms of the pari]tial derivatives $\del_v, \del_t$ (cf. lemma \ref{lemma: derivation_action_on_multiloop_algebras}), one sees thus that indeed, we have that:
                    $$D \in \d_{[2]}$$
                Since $D \in \der_k(A)_{(v, t) = (0, 0)}$ was chosen arbitrarily, this means that:
                    $$\d_{[2]} \supset \der_k(A)_{(v, t) = (0, 0)}$$
