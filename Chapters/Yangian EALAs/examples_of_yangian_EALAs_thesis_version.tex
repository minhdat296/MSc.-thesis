    ...
    \subsection{Which extended toroidal Lie algebras are Yangian ?}
        ...
        \end{proposition}
            \begin{proof}
                ...
                for all $D \in \divzero$. From this, we get that:
                ...
                Solving this system yields:
                    $$\lambda_{\pm r, 0}(D_{\pm r, -1}) = -\frac12 r^2 \mp r$$
        ...
        \todo[inline]{After redoing the proof, it seems that both $\sigma_1$ and $\sigma_2$ are coboundary. What's a bit curious to me is that, $\tau_i$ is very similar to $\tilde{\sigma}_i$ in proposition \ref{prop: non_trivial_yangian_restricted_coboundaries_examples}, but I'm not sure if anything important can be said about this.}
        ...
        \end{theorem}
            \begin{proof}
                ...
                we shall prove a stronger statement, namely the existence of $\tau_i \in C_1(\divzero, \z_{[2]})$ that is \textit{graded}. Should this be true, then through the fact that:
                    $$\deg K_{a, b} = b$$
                    $$\deg c_v = \deg c_t = 0$$
                (cf. remark \ref{remark: Z_gradings_on_toroidal_lie_algebras}), we would have coefficients $\lambda_{r, s}, \alpha'_{r, s}, \alpha_{r, s}, \beta_{r, s} \in \bbC$ such that:
                    $$
                        \begin{aligned}
                ...
                \begin{enumerate}
                    ...
                    From the second and third equations, one sees that:
                        $$\delta_{(r, s), (0, -1)} \alpha_{r, s} = \delta_{(r, s), (0, -1)} r$$
                        $$\delta_{(r, s), (0, -1)} \beta_{r, s} = \delta_{(r, s), (0, -1)} s$$
                    (note that $N_i(r, s, 0, -1) = 0$) and hence:
                        $$\alpha_{r, s} = r$$
                        $$\beta_{r, s} = s$$
                    By putting everything together, we shall obtain:
                        $$\tau_i(D_{r, s}) = ( \delta_{i, 1} r^2 + 2r ) K_{-r, -s - 2} + \delta_{(r, s), (0, -1)} ( r c_v + s c_t )$$
                    regardless of whether $i = 1$ or $i = 2$.
        ...
        \end{remark}
        \begin{remark}[Coboundariness can not be checked on restrictions]
            It is trivial to see that:
                $$\tau_i|_{\frakw} = (\delta_{i, 1} r^2 + 2r) K_{-r, - 1} \not = \delta_{r, 0} c_t - \left(\frac12 r^2 + r\right) K_{r, 0} = \tilde{\sigma}_i$$
            as elements of $C_1(\frakw, \z_{[2]})$, with $\tilde{\sigma}_i$ being as in proposition \ref{prop: non_trivial_yangian_restricted_coboundaries_examples}. As such, we see that even though the restricted $2$-cocycle $\sigma_i|_{\bigwedge^2 \frakw} \in Z^2_{\Lie}(\frakw, \z_{[2]})$ is \textit{not} $2$-coboundary and hence \textit{not} cohomologous to $0$, the \say{full} toroidal $2$-cocycle $\sigma_i \in Z^2_{\Lie}(\divzero, \z_{[2]})$ itself can still be $2$-coboundary.
        \end{remark}
        \begin{remark}
            In example \ref{example: yangian_cocycles_(counter)_examples}, we have seen that the toroidal $2$-cocycle:
                $$\sigma_2$$
